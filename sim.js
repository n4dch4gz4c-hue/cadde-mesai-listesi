(function(){
var LINES=["Semih listeye bakiyor...","Bu HS mi fazla?","Kaydetmeyi unutmayin.","Ozete suzuluyor.","Toplami kontrol etti.","Tamamdir, devam."];
var tIdle=0,running=false,bot=null;
function css(){
  if(document.getElementById("simCss"))return;
  var s=document.createElement("style");
  s.id="simCss";
  s.textContent="#semihBot{position:fixed;z-index:10000;width:86px;pointer-events:none;transition:left .9s ease,top .9s ease;text-align:center}"+ 
  ...
  "@keyframes semihWave{0%,100%{transform:rotate(0)}50%{transform:rotate(-18deg)}}".concat(".semihGlow{outline:3px solid #1f4b8f;outline-offset:2px}");
...
function findSemihEl(){
  var nodes=document.querySelectorAll("td.name, .phd b, .pcard b, h1");
...
function bubble(msg){ if(typeof toast==="function") toast(msg); }
function act(){
  ...
  if(step===0){
    ...
    moveTo(r.left+Math.min(r.width,80), r.top-8);
    ...
    if(typeof reply==="function") reply("Semih: kaydetmeyi unutma.");
    bubble("Asistana fisildadi");
  } else if(step===3){
    var tabEl=document.querySelector('[data-tab="ozet"]');
    if(tabEl){var r=tabEl.getBoundingClientRect(); moveTo(r.left, r.top+30);}
    bubble("Semih ozete goz atti");
  } else {
    moveTo(40+Math.random()*(window.innerWidth-140), 80+Math.random()*220);
    bubble(LINES[Math.floor(Math.random()*LINES.length)]);
  }
}
function loop(){
...
["pointerdown","keydown"].forEach(function(ev){ window.addEventListener(ev,bump,{passive:true}); });
function watch(){
  if(!running && Date.now()-tIdle>16000) start();
  setTimeout(watch, 3000);
}
function boot(){ tIdle=Date.now(); setTimeout(watch, 4000); }
