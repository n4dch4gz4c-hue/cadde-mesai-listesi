...
function rowCalc(p){
  ...
  return {mesai:mesai,hs:hs,prim:prim,yol:yol,izin:izin,yillik:yillik,dis:Number(p.disiplinDk||0),toplam:mesai+hs+prim+yol+izin+yillik+Number(p.disiplinDk||0)};
}
function people(){
  try{ if(window.state&&state.people&&state.people.length) return state.people; }catch(e){}
  try{ if(window.state&&window.state.people&&window.state.people.length) return window.state.people; }catch(e){}
  try{
...
function htmlKisa(list){
  ...
  h="<h2 style='margin:0 0 8px;font-size:18px'>Kisa ozet</h2>";
  ...
  for(i=0;i<list.length;i++){
    p=list[i]; c=rowCalc(p); tot+=c.toplam||0; if(c.toplam>0) pay++;
  ...
  h="<p style='margin:0 0 8px'>"+list.length+" kisi / "+pay+" odeme \u00b7 <b>"+money(tot)+" TL</b></p>"+h;
  return h;
}
function htmlAyr(list){
  var i,p,c,tot=0,mesai=0,hs=0,prim=0,yol=0,izin=0,yillik=0,pay=0,h="";
  h="<h2 style='margin:0 0 8px'>Ayrintili ozet</h2>";
  ...
  return "<p style='margin:0 0 8px'>"+list.length+" kisi / "+pay+" odeme \u00b7 <b>"+money(tot)+" TL</b></p>"+h;
}
function show(list){
